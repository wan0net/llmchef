...
export interface Crea8Frontmatter {
  ...
  created?: string;
  updated?: string;
  "litechat-memory-scope"?: Crea8MemoryScope;
  "litechat-project-id"?: string;
  "litechat-skill-id"?: string;
  [key: string]: unknown;
}
...
export const parseCrea8MarkdownNote = (
  ...
  const scope = parseMemoryScope(frontmatter["litechat-memory-scope"]);
  ...
  return {
    ...
    projectId: optionalString(frontmatter["litechat-project-id"]),
    skillId: optionalString(frontmatter["litechat-skill-id"]),
    path: fallbackPath,
    createdAt,
    updatedAt,
  };
...
export const serializeCrea8MarkdownNote = (note: Crea8MemoryNote): string => {
  const frontmatter: Crea8Frontmatter = {
    ...
    "litechat-memory-scope": note.scope,
  };

  if (note.projectId) frontmatter["litechat-project-id"] = note.projectId;
  if (note.skillId) frontmatter["litechat-skill-id"] = note.skillId;
