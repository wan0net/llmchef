...
import type { Crea8MemoryNote } from "@/types/llmchef/crea8-memory";
...
llmchef-memory-scope: "project"
llmchef-project-id: "project_1"
---

LLMChef deploys at wan0.net/llmchef.
`,
    ...
    expect(note.tags).toEqual(["llmchef", "deploy"]);
    expect(note.content).toBe("LLMChef deploys at wan0.net/llmchef.\n");
  });
  ...
  it("serializes memory notes as crea8-compatible markdown", () => {
    ...
    expect(markdown).toContain('llmchef-memory-scope: "user"');
  ...
  it("creates pending memory proposals", () => {
    const proposal = createMemoryProposal({
      ...
      proposedContent: "LLMChef is deployed at wan0.net/llmchef.",
