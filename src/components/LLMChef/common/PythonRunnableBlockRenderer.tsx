...
class GlobalPythonManager {
  ...
  async ensurePythonLoaded(): Promise<void> {
    // Initialize window.llmChefPython if not exists
    if (!window.llmChefPython) {
      window.llmChefPython = {
        isLoading: false,
        isReady: false,
    ...
    if (window.pyodide && window.llmChefPython.isReady) {
      return;
    }
    ...
    // Start loading
    window.llmChefPython.isLoading = true;
...
_global_capture = GlobalOutputCapture()
      ...
      window.llmChefPython!.isReady = true;
      window.llmChefPython!.isLoading = false;
  ...
  async loadPackages(packages: string[]): Promise<void> {
    ...
    try {
      ...
      window.llmChefPython!.loadedPackages = this.loadedPackages;
  ...
  static dispose(): void {
    ...
    if (window.llmChefPython) {
      window.llmChefPython.isLoading = false;
      window.llmChefPython.isReady = false;
      if (window.llmChefPython.loadedPackages && typeof window.llmChefPython.loadedPackages.clear === 'function') {
        window.llmChefPython.loadedPackages.clear();
      } else {
        window.llmChefPython.loadedPackages = new Set<string>();
      }
      window.llmChefPython.loadPromise = undefined;
    }
  ...
  const executeCode = useCallback(async () => {
    ...
    try {
      ...
      if (hasMatplotlib) {
        ...
        if (!window.llmChefPython?.loadedPackages.has('matplotlib')) {
...
print("✓ Matplotlib configured for Pyodide with always-present target element")
      ...
      window.pyodide.globals.set("llmchef", contextObj.llmchef);
  ...
  const getRunButtonText = () => {
    ...
    if (window.llmChefPython?.isLoading) return t('pythonRunnableBlock.loadingPython');
    if (!window.llmChefPython?.isReady) return t('pythonRunnableBlock.loadPython');
    ...
    <div className="code-block-container group/codeblock my-4 max-w-full">
      <div className="code-block-header sticky top-0 z-[var(--z-sticky)] flex items-center justify-between px-3 py-2 border border-b-0 border-border bg-muted/50 rounded-t-lg">
        <div className="flex items-center gap-1">
          ...
          {window.llmChefPython?.isReady && (
            <div className="text-xs text-green-600 bg-green-100 px-2 py-0.5 rounded">
{t('pythonRunnableBlock.ready', { count: window.llmChefPython.loadedPackages.size })}
            </div>
          )}
          {securityResult && (
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            disabled={isRunning || window.llmChefPython?.isLoading || !runnableBlocksEnabled}
            className={
              `text-xs h-7 ` +
              (securityResult
          ...
          >
            {isRunning || window.llmChefPython?.isLoading ? (
              <Loader2Icon className="h-3 w-3 mr-1 animate-spin" />
            ) : !window.llmChefPython?.isReady ? (
