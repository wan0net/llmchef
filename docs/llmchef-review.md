...
- Model-list fetching, optional web search/content extraction, marketplace downloads, remote mod scripts, MCP HTTP/bridge calls, OpenRouter endpoint metadata, race-export stylesheet capture, and Formedible location lookups now pass through `src/lib/litechat/outbound-policy.ts`, which blocks non-HTTP(S) URLs, enforces known provider hosts for model-list requests, and keeps an in-memory destination log for future UI surfacing.
