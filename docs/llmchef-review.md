...
- Model-list fetching and optional web search/content extraction now pass through `src/lib/litechat/outbound-policy.ts`, which blocks non-HTTP(S) URLs, enforces known provider hosts for model-list requests, and keeps an in-memory destination log for future UI surfacing.
