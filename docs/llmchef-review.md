...
- The build now uses explicit Vite vendor chunks for React, AI SDK/provider adapters, storage/git, editor, diagram, and UI dependencies. Settings tab bodies, advanced block renderers, and heavier modal/panel controls are lazy-loaded when opened or encountered. Browser Node polyfills were removed after the production build passed without them. Best next reductions are moving more optional prompt controls behind dynamic imports and evaluating Mermaid alternatives.
